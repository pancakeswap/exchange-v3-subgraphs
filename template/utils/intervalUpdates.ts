 ...
 * @param event
 */
export function updatePancakeDayData(factory: Factory, event: ethereum.Event): PancakeDayData {
  ...
  pancakeDayData.tvlUSD = factory.totalValueLockedUSD
  pancakeDayData.txCount = factory.txCount
  pancakeDayData.save()
...
export function updateTokenDayData(bundle: Bundle, token: Token, event: ethereum.Event): TokenDayData {
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400
  let dayStartTimestamp = dayID * 86400
...
export function updateTokenHourData(bundle: Bundle, token: Token, event: ethereum.Event): TokenHourData {
  let timestamp = event.block.timestamp.toI32()
  let hourIndex = timestamp / 3600 // get unique hour within unix history
  let hourStartUnix = hourIndex * 3600 // want the rounded effect
